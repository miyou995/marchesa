...
class Cart(object):
    ...
    def add(self, product, taille=False, color=False,pointure=False, quantity=1, override_quantity=False):
        product_id = str(product.id)
        if product_id not in self.cart:
            self.cart[product_id] = {'quantity': 0, 'price': str(product.price), 'taille' : taille,'color' : color, 'pointure':pointure }
        if override_quantity:
            self.cart[product_id]['quantity'] = quantity
        else:
            self.cart[product_id]['quantity'] += quantity
